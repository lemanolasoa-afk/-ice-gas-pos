...
export function CartPage() {
  ...
  const { cart, getTotal, getDepositTotal, clearCart } = useStore()
  const baseTotal = getTotal()
  const depositTotal = getDepositTotal()
  const total = baseTotal + depositTotal
  ...
  return (
    <div className="min-h-screen pb-48 bg-gray-50">
      {/* Header */}
      <header className="bg-white border-b border-gray-100 px-4 py-3 sticky top-0 z-10">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-gray-500">{itemCount} รายการ • รวม <span className="font-semibold text-gray-700">฿{total.toLocaleString()}</span></p>
          </div>
          {cart.length > 0 && (
            <button
              onClick={() => {
        ...
        <div className="fixed bottom-16 left-0 right-0 bg-white border-t border-gray-200 shadow-lg z-10">
          <div className="max-w-lg mx-auto p-3">
            ...
            <div className="flex items-center justify-between bg-gray-50 rounded-xl p-3 mb-2">
              <div>
                <p className="text-sm text-gray-500">{itemCount} รายการ</p>
                <p className="text-xl font-bold text-gray-800">฿{total.toLocaleString()}</p>
              </div>
              <button
                onClick={() => navigate('/')}
                className="px-4 py-2 bg-white border border-gray-200 text-gray-700 rounded-lg font-medium text-sm flex items-center gap-1.5 shadow-sm"
              >
                <Plus size={16} />
                เพิ่มสินค้า
              </button>
            ...
            <button
              ...
              className="w-full py-3.5 bg-green-600 text-white rounded-xl font-bold text-lg hover:bg-green-700 transition-colors shadow-sm"
            >
